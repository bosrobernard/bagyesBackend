...
export class OrderService {
  static async createOrder(
    ...
    try {
      const { trackingId, ...safeOrderData } = orderData as any;
      const order = new Order({
        ...safeOrderData,
        createdBy,
        trackingId: generateTrackingId(), // Set explicitly
      });
      logger.info("Order before save:", order.toObject());
      await order.save();
      logger.info("Order after save:", order.toObject());
    ...
    try {
      ...
      order.timeline.push({
        ...
        description:
          ...
          this.getStatusDescription(statusData.status),
        timestamp: new Date(),
      });

      await order.save();
      logger.info(
        `Order ${trackingId} status updated to ${statusData.status} by user ${updatedBy}`
      );
